...
public:
    ...
    float shininess;
    // ctor/dtor
    Sphere(float radius = 1.0f, int sectorCount = 36, int stackCount = 18, glm::vec3 amb = glm::vec3(0.0f, 1.0f, 0.0f), glm::vec3 diff = glm::vec3(0.0f, 1.0f, 0.0f), glm::vec3 spec = glm::vec3(0.5f, 0.5f, 0.5f), float shiny = 32.0f) : verticesStride(24)
    {
        set(radius, sectorCount, stackCount, amb, diff, spec, shiny);
        buildCoordinatesAndIndices();
        buildVertices();

        textureID = loadTexture("C:/Users/mifth/OneDrive/Desktop/4-2'18/2. Lab/CSE 4208 (Graphics Lab)/Assignment_B1/B1_3/Lighting - Copy/grass.png", true);
    ...
    void drawSphere(Shader& lightingShaderWithTexture, glm::mat4 model) const
    {
        ...
        lightingShaderWithTexture.setInt("textureSampler", 0); // 0 is the texture unit index
...
private:
    ...
    void buildCoordinatesAndIndices()
    {
        ...
        for (int i = 0; i <= stackCount; ++i)
        {
            ...
            for (int j = 0; j <= sectorCount; ++j)
            {
                ...
                // calculate texture coordinates and add to vector
    ...
    void buildVertices()
    {
        ...
        for (i = 0, j = 0; i < count; i += 3, j += 2)
        {
            ...
            /*vertices.push_back(textureCoords[j]);
            vertices.push_back(textureCoords[j + 1]);*/
        }
    }
...
GLuint Sphere::loadTexture(const char* fileName, bool wrap) {
